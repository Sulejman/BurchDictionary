...
public class ShowDetailsActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		final PhraseAdapter wordAdapter = new PhraseAdapter(ShowDetailsActivity.this, showList() );
		...
		bosnianPr.setText("["+c.getString(c.getColumnIndex("BOSNIAN_RONUNCIATION"))+"]");
		turkishPr.setText("["+c.getString(c.getColumnIndex("TURKISH_PRONUNCIATION"))+"]");
		c.close();
	}
	...
	private List<Word> showList() {
		ArrayList<Word> words = new ArrayList<Word>();
		  ...
		  for(int i=1; i<=c1.getCount();i++){
			  ...
			  if(i == 1){
				  if(c5.moveToFirst()){
					  TrBs = c5.getString(c5.getColumnIndex("TURKISH_PHRASE_BOSNIAN_PRONUNCIATION"));
					  }
				  else
					  TrBs=null;}
				  else{
			  ...
			  if(i == 1){
				  if(c4.moveToFirst()){
					  BsTr = c4.getString(c4.getColumnIndex("BOSNIAN_PHRASE_TURKISH_PRONUNCIATION"));
					  }
				  else
					  BsTr=null;}
				  else{
